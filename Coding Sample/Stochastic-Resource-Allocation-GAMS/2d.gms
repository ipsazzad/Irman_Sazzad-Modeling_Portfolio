...
PARAMETERS
    ...
    EXPORTTAX Export tax per unit shipped between islands /50/;
...
* Surplus equation for each island
ISURP(I).. 
    SURP(I) =E= A*CONS(I) - B*CONS(I)*CONS(I)/2;

* Total transport costs, including export tax
ETRANS.. 
    TTRANS =E= SUM(I, SUM(IP, (TCOST * DIST(I, IP) + EXPORTTAX) * X(I, IP)));

* Price determination for each island
PRICE(I).. 
    P(I) =E= A - B*CONS(I);

* Consumption on each island
CON(I).. 
