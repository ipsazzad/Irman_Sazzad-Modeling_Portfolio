...
*Factor demands
LAB_p_eq(i,tt) .. LAB_p(i,tt) =e= (QD_p(i,tt)/(theta(i)*shkk(i,tt)))*(((1-beta(i))/w_p(tt))**sigmap(i))*(((beta(i)**sigmap(i))*(r_p(tt)**(1-sigmap(i)))+((1-beta(i))**sigmap(i))*(w_p(tt)**(1-sigmap(i))))**(sigmap(i)/(1-sigmap(i))));
CAP_p_eq(i,tt) .. CAP_p(i,tt) =e=(QD_p(i,tt)/(theta(i)*shkk(i,tt)))*(((beta(i))/r_p(tt))**sigmap(i))*(((beta(i)**sigmap(i))*(r_p(tt)**(1-sigmap(i)))+((1-beta(i))**sigmap(i))*(w_p(tt)**(1-sigmap(i))))**(sigmap(i)/(1-sigmap(i))));
*trade
